...
import { Upload, Bell, Clock, ExternalLink, AlertCircle } from 'lucide-react';
...
export default function Publicacoes() {
  ...
  const filtered = state.publicacoes.filter(p => {
    ...
    const matchSearch = !search || p.tribunal.toLowerCase().includes(search.toLowerCase()) || p.numeroProcesso.includes(search) || p.conteudo.toLowerCase().includes(search.toLowerCase());
  ...
  const handleGerarPrazo = (pubId: string) => {
    ...
    dispatch({ type: 'ADD_PRAZO', payload: { id: genId(), processoId: pub.processoId || '', tipo: 'prazo_fatal', descricao: prazoDescricao, dataHora: prazoData + 'T23:59', diasUteis: true, responsavel: state.advogados[0]?.nome || '', status: 'pendente', alertaDias: 3, criadoEm: new Date().toISOString() } });
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          {naoLidas > 0 && <p className="text-sm text-red-600 flex items-center gap-1 mt-0.5"><Bell size={12} />{naoLidas} não lida(s)</p>}
        </div>
      ...
      <Tabs defaultValue="publicacoes">
        ...
        <TabsContent value="publicacoes" className="mt-4 space-y-4">
          <div className="flex gap-2 flex-wrap">
            <Input className="h-8 text-sm flex-1 min-w-48" placeholder="Buscar por tribunal, nº processo, conteúdo..." value={search} onChange={e => setSearch(e.target.value)} />
          ...
          <div className="space-y-3">
            {filtered.length === 0 && <p className="text-sm text-gray-500 py-8 text-center">Nenhuma publicação encontrada.</p>}
            {filtered.map(pub => {
              const proc = state.processos.find(p => p.id === pub.processoId);
              return (
                <Card key={pub.id} className={`cursor-pointer hover:shadow-md transition-shadow ${pub.status === 'não_lida' ? 'border-red-200' : ''}`} onClick={() => marcarLida(pub.id)}>
                  <CardContent className="p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-start gap-3 min-w-0 flex-1">
                        <div className={`w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${pub.status === 'não_lida' ? 'bg-red-500' : 'bg-gray-300'}`} />
                      ...
                      <div className="flex flex-col gap-1.5 flex-shrink-0">
                        ...
                        {pub.status !== 'prazo_gerado' && (
                          <Button size="sm" className="h-7 text-[10px] bg-[#1e3a5f] hover:bg-[#2563eb] px-2" onClick={e => { e.stopPropagation(); setGerarPrazoId(pub.id); }}>
      ...
      <Dialog open={!!gerarPrazoId} onOpenChange={() => setGerarPrazoId(null)}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3">
            <div><Label className="text-xs">Descrição do Prazo *</Label><Input className="mt-1 h-8 text-sm" value={prazoDescricao} onChange={e => setPrazoDescricao(e.target.value)} placeholder="Ex: Contrarrazões de recurso" /></div>
            <div><Label className="text-xs">Data Limite *</Label><Input type="date" className="mt-1 h-8 text-sm" value={prazoData} onChange={e => setPrazoData(e.target.value)} /></div>
          </div>
